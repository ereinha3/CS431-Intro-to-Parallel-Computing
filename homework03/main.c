                        ...
                        double** csr_vals)
{
    ...
    *csr_row_ptr = calloc(m + 1, sizeof(unsigned int));
              ...
              omp_lock_t* writelock)
{
	...
	for (int i = 0; i<nnz; i++){
		...
		double result = value * vector_x[col];
          ...
          double* vector_x, double *res)
{
	...
	#pragma omp parallel for
		...
		int iterable = csr_row_ptr[row+1]-csr_row_ptr[row];
        	for (int idx = 0; idx < iterable; idx++) {
            		int cur_index = csr_row_ptr[row] + idx;
			int col = csr_col_ind[cur_index];
            		res[row] += csr_vals[cur_index] * vector_x[col];
        	}
    	}
